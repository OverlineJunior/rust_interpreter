    ...
    I: ValueInput<'a, Token = Token, Span = SimpleSpan>,
{
    ...
    let expr = {
        ...
        let product = unary.clone().foldl(
            choice((
                just(Token::Mul).to(Expr::Mul as fn(_, _) -> Expr),
                just(Token::Div).to(Expr::Div as fn(_, _) -> Expr),
            ))
            .then(unary)
            .repeated(),
        ...
        let sum = product.clone().foldl(
            choice((
                just(Token::Add).to(Expr::Add as fn(_, _) -> Expr),
                just(Token::Sub).to(Expr::Sub as fn(_, _) -> Expr),
            ))
            .then(product)
            .repeated(),
            |lhs, (op, rhs)| op(Box::new(lhs), Box::new(rhs)),
